...
import { fetchAudio } from "../messages/speakCharacter";
...
if (typeof window !== "undefined") {
  ...
  (window as any).testMockLLMMessageWithAudio = async (
    ...
    console.log("Note: Expression is determined by LLM tags, NOT by audio content!");
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      try {
        audioBuffer = await fetchAudio(screenplay.talk, apiKey, elevenLabsParam);
    ...
    console.log("\n📝 Key Points:");
    console.log("  - Expression (happy/sad) is determined by LLM tags [happy], [sad], etc.");
    console.log("  - Audio does NOT determine expression - it only drives lip sync (mouth movement)");
    console.log("  - Lip sync analyzes audio volume in real-time to open/close mouth");
  };
}
