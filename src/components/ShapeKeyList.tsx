...
export function ShapeKeyList() {
  ...
  const [shapeKeyValues, setShapeKeyValues] = useState<{ [key: string]: number }>({});
  const [morphTargetMap, setMorphTargetMap] = useState<Map<string, Array<{ mesh: THREE.SkinnedMesh; index: number }>>>(new Map());
  ...
  useEffect(() => {
    if (viewer.model?.vrm?.scene) {
      ...
      if (vrm.expressionManager && (vrm.expressionManager as any).blendShapeProxy) {
        const exprBlendShapeProxy = (vrm.expressionManager as any).blendShapeProxy;
        const exprBlendShapeGroups = exprBlendShapeProxy?.blendShapeGroups || [];
        ...
        if ((object as any).isMesh) {
          ...
          if (geometry.morphAttributes?.position) {
            ...
            // Method 2: Check morphTargetDictionary (standard Three.js)
              ...
              targetNames = Object.keys((geometry as any).morphTargetDictionary);
            }
      ...
      sortedKeys.forEach((key) => {
        ...
        if (targets && targets.length > 0) {
          ...
          if ((firstTarget as any).isVRM1 && (firstTarget as any).blendShapeGroup) {
            ...
            const fromExpressionManager = (firstTarget as any).fromExpressionManager;
            
            // Try expression manager's blendShapeProxy first if it came from there
            let blendShapeProxy = fromExpressionManager && vrm.expressionManager 
            ...
            if (blendShapeProxy) {
              ...
              initialValues[key] = group.weight !== undefined ? group.weight : 0;
            } else {
              initialValues[key] = 0;
            }
          } else {
            // For VRM 0.x, get value from morphTargetInfluences
            initialValues[key] = firstTarget.mesh.morphTargetInfluences?.[firstTarget.index] ?? 0;
          }
        } else {
          initialValues[key] = 0;
        }
    ...
    try {
      ...
      if ((firstTarget as any).isVRM1 && (firstTarget as any).blendShapeGroup) {
        ...
        targets.forEach(({ mesh, index }) => {
          if (mesh && mesh.morphTargetInfluences && index >= 0 && index < mesh.morphTargetInfluences.length) {
  ...
  return (
    <div className="fixed top-4 left-4 z-20 bg-base border-2 border-primary rounded-8 shadow-lg max-w-xs w-80" style={{ marginTop: shapeKeys.length > 0 ? '320px' : '60px' }}>
      <button
        ...
        <div className="border-t border-primary flex flex-col" style={{ maxHeight: 'calc(100vh - 400px)' }}>
          {shapeKeys.length === 0 ? (
            <div className="p-8 text-text-secondary text-center typography-14">
              {viewer.model?.vrm ? "No shape keys found" : "Load a VRM model first"}
            </div>
          ) : (
            <>
