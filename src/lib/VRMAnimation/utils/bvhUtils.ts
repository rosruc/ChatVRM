...
export function retargetBVHToVRM(
    ...
    for (const track of bvhClip.tracks) {
        ...
        const vrmBone = vrm.humanoid?.getNormalizedBoneNode(bvhBoneName as VRMHumanBoneName);
        if (!vrmBone) continue;
    ...
    return new THREE.AnimationClip(bvhClip.name, bvhClip.duration, retargetedTracks);
}
...
export function calculateVRMHipsHeight(vrm: VRM): number {
    ...
    return vrmHipsBone 
        ? Math.abs(vrmHipsBone.position.y - vrm.scene.position.y)
        : 1.0;
}
